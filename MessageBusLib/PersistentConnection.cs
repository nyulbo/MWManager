...
namespace MessageBus
{
    ...
    public class PersistentConnection : IPersistentConnection
    {
        ...
        public PersistentConnection(IConnectionFactory connectionFactory, ILogger logger, IEventBus eventBus)
        {
        ...
        public void Initialize()
        {
            lock (locker)
            {
                if (initialized)
                {   
                    throw new MessageBusException("This PersistentConnection has already been initialized.");
                }
                initialized = true;
                TryToConnect(null);
            }
        ...
        public IModel CreateModel()
        {
            if (!IsConnected)
            {   
                throw new MessageBusException("PersistentConnection: Attempt to create a channel while being disconnected.");
            }
