    ...
    public class DefaultServiceProvider : IContainer
    {
        ...
        public virtual TService Resolve<TService>() where TService : class
        {
            ...
            lock (syncLock)
            {
                ...
                {   
                    throw new MessageBusException("No service of type {0} has been registered", serviceType.Name);
                }
                return (TService)service;
            }
        }
            ...
            where TImplementation : class, TService
        {
            lock (syncLock)
            {
                ...
                {
                    throw new MessageBusException("Component {0} does not implement service interface {1}",
                ...
                {
                    throw new MessageBusException("An MessageBus service must have one and one only constructor. " +
