...
namespace MessageBus.Consumer
{
    ...
    public class BasicConsumer : IBasicConsumer, IDisposable
    {
        ...
        public BasicConsumer(Action<BasicConsumer> cancelled, IConsumerDispatcher consumerDispatcher, IQueue queue, IEventBus eventBus, 
            IHandlerRunner handlerRunner, Func<byte[], MessageProperties, MessageReceivedInfo, Task> onMessage, ILogger logger, IModel model)
        {
            Queue = queue;
            OnMessage = onMessage;
        ...
        public void HandleBasicConsumeOk(string consumerTag)
        {
            ...
            logger.InfoWrite("BasicCancel(Consumer Ok Notification from broker) event received. " +
        ...
        public void HandleModelShutdown(object model, ShutdownEventArgs reason)
        {
            logger.InfoWrite("Consumer '{0}', consuming from queue '{1}', has shutdown. Reason: '{2}'",
                ConsumerTag, Queue.Name, reason.Cause + " : " + reason.ReplyText );
        }
